...
| `GITHUB_MEMBER_SUFFIX` | A suffix appended to each team member username before matching/adding them on GitHub. Takes precedence over `spec.memberSuffix` on the `Organization` CR. Prefer using `spec.memberSuffix` for per-organization suffixes in multi-org setups. | Any string (e.g. `"_company-name"`, `""`) | `""` (no suffix) |
